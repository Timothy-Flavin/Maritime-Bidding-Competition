...
class Group8Company(TradingCompany):
    def __init__(self, fleet, name):
        ...
        self._won_trades = []
        self.simulated_annealing = SAScheduler()  # Initialize simulated annealing
    ...
    def inform(self, trades, *args, **kwargs):
        ...
        self._current_scheduling_proposal = self.build_schedule_proposal(best_schedule_string, trades)

        # Step 4: Bid infinite for each scheduled trade
        bids = [Bid(amount=float('inf'), trade=trade) for trade in self._current_scheduling_proposal.scheduled_trades]
        return bids
    ...
    def propose_schedules(self, date):
        ...
        if self._current_scheduling_proposal is None:
            raise ValueError("No schedule proposal ready when propose_schedules() was called!")
